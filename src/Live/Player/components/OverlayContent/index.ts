import { LivePlayerConfig } from '../../../../common/types';
 ...
 * @param {Object} config data to hide overlay elements.
 */
export default function (episode, config: LivePlayerConfig): string {
  const { show } = episode;
  const { hideTitle, hideLiveLogo, hideShoppingCart, hideDescription } = config;
  ...
  function renderProductsContainer() {
    return hideShoppingCart ? '' : featuredProductsContainer();
  }
